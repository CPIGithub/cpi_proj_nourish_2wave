...
Purpose				:	Endline data collection: import village raw data into dta format 				
...
*******************************************************************************/
	...
	********************************************************************************
	...
	import excel using "$raw/endline/PN_Village_Survey_ENDLINE_FINAL.xlsx", describe
	...
	forvalue x = 1/`r(N_worksheet)' {
		...
		import excel using "$raw/endline/PN_Village_Survey_ENDLINE_FINAL.xlsx", sheet("`sheet_`x''") firstrow clear 
	...
	* apply WB codebook command 
	//iecodebook template using "$raw/endline/codebook/codebook_village_svy.xlsx", replace 
	iecodebook apply using "$raw/endline/codebook/codebook_village_svy.xlsx"
	...
	** demo_migrate_rep - migration information 
	...
	if `r(N)' > 0  {
			...
			preserve
			use "$dta/endline/demo_migrate_rep.dta", clear
			...
			* save as labeled var dataset 
			save "$dta/endline/demo_migrate_rep_label.dta", replace 
	...
	** demo_idp_rep - IDP information 
	...
	if `r(N)' > 0  {
			...
			preserve
			use "$dta/endline/demo_idp_rep.dta", clear
			...
			* save as labeled var dataset 
			save "$dta/endline/demo_idp_rep_label.dta", replace 
	...
	** demo_dspl_rpt - displaced population information not included yet 
	...
	if `r(N)' > 0  {
			...
			preserve
			use "$dta/endline/demo_dspl_rpt.dta", clear
			...
			* save as labeled var dataset 
			save "$dta/endline/demo_dspl_rpt_label.dta", replace 
	...
	** lh_rpt - liveihood information 
	...
	if `r(N)' > 0  {
			...
			preserve
			use "$dta/endline/lh_rpt.dta", clear
			...
			* save as labeled var dataset 
			save "$dta/endline/lh_rpt_label.dta", replace 
	...
	** pn_dev_rpt - development project info 
	...
	if `r(N)' > 0  {
			...
			preserve
			use "$dta/endline/pn_dev_rpt.dta", clear
	...
	save "$dta/endline/pnourish_endline_village_svy_wide.dta", replace  
