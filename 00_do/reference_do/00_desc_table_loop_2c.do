set trace on	
		forvalue m=1(1)`=scalar(numberoftables)' { 
			...
			quietly foreach j of varlist ${${matrixname`m'}tablevars} {
				...
				if `ifnotcat' {			/* ifnotcat local defined above; contains all categorical variables */
					...
					matrix ${matrixname`m'}[`i',`col'+3]=r(N)
					...
					sum `j' if temp==2
					local c2obs`j'=r(N)
					if `allobs`j''!=0 & `c1obs`j''!=0 & `c2obs`j''!=0 {
					...
					foreach l of local levels {
						...
						forvalue c=1(1)2 {
							...
							matrix ${matrixname`m'}[`i',`col'+4]=r(N)
							local col=`col'+4
						}
						if `l'==`lastlevel' {
							local col=`col'+2
			...
			matrix ${matrixname`m'}[1,4]=`obsallmax' /* write max sample size to top of each column */
			forvalue c=1(1)2 {
				matrix ${matrixname`m'}[1,4+`c'*4]=`obsc`c'max'
			}
			
			*** generate sample size footnote ***
			local footnotenecessary=0
			global footnote "${footnote}Sample size at baseline ranged from "
		...
		*** name rows/cols; transfer to data set ***
			...
			mat coln ${matrixname`m'}=%ma sda p50a #a %m1 sd1 p501 #1 %m2 sd2 p502 #2 se p
			noisily mat list ${matrixname`m'}
			display "$footnote" 
		
			svmat2 ${matrixname`m'}, names(pctma sda p50a na pctm1 sd1 p501 n1 pctm2 sd2 p502 n2 se p)
			...
			** extract var names differently **
			...
			keep vars pctma sda na pctm1 sd1 n1 pctm2 sd2 n2 se p 
			order vars pctma sda na pctm1 sd1 n1 pctm2 sd2 n2 se p 
			format %9.1f pc* sd*
			format %9.3f p
