...
export excel title using "$out/endline/03_hfc_hh_outlier.xlsx", sheet("01_SUMMARY") sheetreplace cell(A1) 
...
export excel title using "$out/endline/03_hfc_hh_outlier.xlsx", sheet("01_SUMMARY") sheetmodify cell(A6) 
...
import excel using "$dir/01_sampling/02_Questionnaires/FINAL/ENDLINE/Pnourish_ENDLINE_HH_Survey.xlsx", sheet("survey") firstrow clear 
...
foreach v of varlist $interestedvar {
	...
	//if !_rc {
			...
			* replace the don'know values as missing 
			replace `v' = .m if `v' == 444 | `v' == 999 |  `v' == 777 | `v' == 888
			sum `v', d
...
export excel using "$out/endline/03_hfc_hh_outlier.xlsx", sheet("01_SUMMARY") firstrow(varlabels) cell(A2) keepcellfmt sheetmodify
...
export excel using "$out/endline/03_hfc_hh_outlier.xlsx", sheet("01_SUMMARY") firstrow(varlabels) cell(A7) keepcellfmt sheetmodify
...
if _N > 0 {
	...
	export excel using "$out/endline/03_hfc_hh_outlier.xlsx", sheet("02_DETAILED_OBS") firstrow(varlabels) keepcellfmt sheetreplace
}
