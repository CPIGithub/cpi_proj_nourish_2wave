...
Purpose				:	Endline data collection: enumerator performance			
...
export excel title using "$out/endline/06_hcf_enu_performance.xlsx", sheet("01_SUMMARY") sheetreplace cell(A1) 
...
export excel title using "$out/endline/06_hcf_enu_performance.xlsx", sheet("01_SUMMARY") sheetmodify cell(A6) 
...
gen svy_dur_long = (svy_dur > 120 /*(svy_dur_mean + (`r(sd)' * 2))*/)
...
gen svy_complet_diffday = svy_date != svy_end_date
...
keep if svy_dur_long == 1 | svy_dur_short == 1 | ///
		svy_early == 1 | svy_late == 1 | svy_complet_diffday == 1
...
egen tot_diffday 		= total(svy_complet_diffday)
gen tot_diffday_shared	= round(tot_diffday/tot_obs, 0.001)
...
keep 	tot_obs tot_flag tot_flag_shared tot_long tot_long_shared tot_short tot_short_shared ///
		tot_diffday tot_diffday_shared tot_early tot_early_shared tot_late tot_late_shared
...
lab var tot_long			"Total long survey obs (> 2 hrs)"
lab var tot_short			"Total short survey obs (< 10 min)" 
...
lab var tot_diffday			"Total more than one day survey" 
lab var tot_diffday_shared	"More than one day survey share"
...
export excel using "$out/endline/06_hcf_enu_performance.xlsx", sheet("01_SUMMARY") firstrow(varlabels) cell(A2) keepcellfmt sheetmodify
...
export excel using "$out/endline/06_hcf_enu_performance.xlsx", sheet("01_SUMMARY") firstrow(varlabels) cell(A7) keepcellfmt sheetmodify
...
keep	township_name geo_eho_vt_name geo_eho_vill_name stratum ///
		...
		svy_complet_diffday svy_early svy_late
...
lab var svy_complet_diffday	"Survey required more than one day to finish"
lab var uuid				"Survey key id - uuid"

if _N {
    
	export excel using "$out/endline/06_hcf_enu_performance.xlsx", sheet("02_DETAILED_OBS") firstrow(varlabels) keepcellfmt sheetreplace
