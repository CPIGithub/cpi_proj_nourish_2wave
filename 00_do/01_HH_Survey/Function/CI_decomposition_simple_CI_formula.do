	...
	* Step 1: Estimate CI of outcome
	...
	sca m_$outcome_var = r(mean)    

	corr rank $outcome_var [aw = weight_final], c // rank - multivar rank 
	sca cov_$outcome_var = r(cov_12) // replace with same CI formula use for outcome var
	sca CI = 2 * cov_$outcome_var / m_$outcome_var  // replace with same CI formula use for outcome var

	svy: qui probit $outcome_var $X 
	qui dprobit $outcome_var $X [pw = weight_var]
