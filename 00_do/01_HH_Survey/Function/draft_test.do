...
svy: proportion pn_yes
matrix table = r(table)

// Display the estimated proportions
matrix list r(table)

// Access a specific proportion (e.g., for the first category)
matrix results = e(b)
local proportion = (results[1,2])

// Calculate the standard error for the first category
matrix V = e(V) 
local se = sqrt(V[1,1])


local p_se = sqrt(`proportion' * (1 - `proportion') / `e(N)')


// Calculate confidence intervals (assuming 95% confidence level)
local lb = `proportion' - 1.96 * `se'
local ub = `proportion' + 1.96 * `se'

local p_lb = `proportion' - 1.96 * `p_se'
local p_ub = `proportion' + 1.96 * `p_se'



di "`proportion' , `se',  `lb', `ub' , `p_se',  `p_lb', `p_ub'"
