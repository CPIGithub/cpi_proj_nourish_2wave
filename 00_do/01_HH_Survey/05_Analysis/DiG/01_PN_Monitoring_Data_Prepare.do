...
Purpose				:	import project monitoring data				
...
*******************************************************************************/
	...
	* get pop data 
	merge m:1 township_name geo_eho_vt_name geo_eho_vill_name using 
	...
	** attendance percent by tot pop
	...
	foreach var in `groups'  {
		...
		gen `var'_pct = round((`var'_max/pop_tot), 0.1)
	...
	* Standardising input variables
	foreach var of varlist attend_tot_pct attend_m_pct attend_f_pct vill_sbcc_tot unique_topic {
		...
		// Z-score - based on not-hired group Mean and SD 
		zindex `var', gen(`var'_z) condition(treat_status == 0) 
		sum `var'*, d
	}
	
	
	* ICW - index 
	icw_index *z, gen(f_pction_fagency_icw)
	
	sum f_pction_fagency_icw, d 
